import React from 'react'

const Schedule = () => {
  return (
    <div className="flex flex-col items-center pt-20  bg-no-repeat bg-center h-[60rem] bg-[#00164D]" style={{}} >
      <div className="title-font-style">
        Mint Schedule
      </div>
        <div className="flex max-w-6xl mt-12">
            <div className="columns-sm">
                <div className="text-right middle-text-style mt-6">
                    Mint
                </div>
                <div className="text-right middle-text-style mt-32  middle-right-title-style-hover">
                    Raffle and Airdrop
                </div>
                <div className="text-right middle-text-style mt-24 md:mt-32 lg:mt-32 no-select-font">
                    Refund
                </div>
                <div className="text-right middle-text-style mt-32 no-select-font">
                    Thanks
                </div>
            </div>
            <div className="columns-xs bg-red ">
                <div className="flex flex-col items-center">
                    <div className="xian xian-hover xian-1"></div>
                    <div className="rounded-full qiu-hover flex items-center justify-center	">
                        <svg width="14" height="10" viewBox="0 0 14 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M13.7068 1.76591L6.04048 9.69683C5.85938 9.88404 5.60926 10 5.33302 10C5.05678 10 4.80667 9.88404 4.6255 9.69683L0.292999 5.21383C0.112011 5.02751 0 4.76871 0 4.48293C0 3.91138 0.447913 3.44883 0.999533 3.44883C1.27577 3.44883 1.526 3.56468 1.7071 3.75193L5.333 7.50217L12.2929 0.302213C12.474 0.115895 12.7241 0 13.0003 0C13.5518 0 14 0.4634 14 1.034C14 1.31982 13.8879 1.57866 13.7068 1.76591Z" fill="white"/>
                        </svg>
                    </div>
                    <div className="xian xian-hover xian-2"></div>
                    <div className="rounded-full qiu-hover flex items-center justify-center	">
                        <svg width="14" height="10" viewBox="0 0 14 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M13.7068 1.76591L6.04048 9.69683C5.85938 9.88404 5.60926 10 5.33302 10C5.05678 10 4.80667 9.88404 4.6255 9.69683L0.292999 5.21383C0.112011 5.02751 0 4.76871 0 4.48293C0 3.91138 0.447913 3.44883 0.999533 3.44883C1.27577 3.44883 1.526 3.56468 1.7071 3.75193L5.333 7.50217L12.2929 0.302213C12.474 0.115895 12.7241 0 13.0003 0C13.5518 0 14 0.4634 14 1.034C14 1.31982 13.8879 1.57866 13.7068 1.76591Z" fill="white"/>
                        </svg>
                    </div>
                    <div className="xian xian-3"></div>
                    <div className="rounded-full qiu"></div>
                    <div className="xian xian-4"></div>
                    <div className="rounded-full qiu"></div>
                    <div className="xian xian-5"></div>
                </div>

            </div>
            <div className="columns-sm text-left">
                <div className=" flex flex-col">
                    <div className="middle-right-main  mt-6 mb-2">
                        <div className="middle-right-title-style">
                            Feb 1,8 PM - Feb 3, 8 PM (UTC+8)
                        </div>
                        <div className="middle-right-desc-style">
                            From the first orders, I did only “stage” and “date” elements at the Roadmap as at many examples before.
                        </div>
                    </div>
                    <div className="middle-right-main mt-8 md:mt-20 lg:mt-20">
                        <div className="middle-right-title-style middle-right-title-style-hover">
                            Feb 1,8 PM - Feb 3, 8 PM (UTC+8)
                        </div>
                        <div className="middle-right-desc-style middle-right-desc-style-hover">
                            Especially for them, I decided to show you version often used in my practice. And what solutions have you found in such moments?
                        </div>
                    </div>
                    <div className="middle-right-main mt-8 md:mt-16 lg:mt-16 md:mb-2 lg:mb-2">
                        <div className="middle-right-title-style no-select-font">
                            Feb 1,8 PM - Feb 3, 8 PM (UTC+8)
                        </div>
                        <div className="middle-right-desc-style no-select-font">
                            i hope you liked. get any feedback to me about this present.
                        </div>
                    </div>
                    <div className="middle-right-main mt-16 md:mt-20 lg:mt-20">
                        <div className="middle-right-title-style no-select-font">
                            Feb 1,8 PM - Feb 3, 8 PM (UTC+8)
                        </div>
                        <div className="middle-right-desc-style no-select-font">
                            Thanks for checking it out. Please press "L" to motivate me to keep shooting. It's free for you but means a lot to me.
                        </div>
                    </div>
                </div>

            </div>
        </div>


    </div>
  )
}

export default Schedule